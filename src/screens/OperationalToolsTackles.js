...
import OperationalInstrument from "./OperationalInstrument";
...
export default function OperationalToolsTackles({ navigation }) {
  // Keep back arrow from showing
  React.useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft: () => null,
      headerTitle: 'Tools And Tackles',
    });
  ...
  const onSubmit = (data) => {
    const tooltacklesData = formData.map((item, index) => {
      ...
      return {
        TooltacklesId: selectedDescriptionId, // Assuming you want to name it ConsumablesId
    ...
    if (tooltacklesData.length > 0) {
      WizardStore.update((s) => {
        s.progress = "1";
        s.tooltacklesData = tooltacklesData;
      });
      navigation.navigate("OperationalInstrument");
    } else {
